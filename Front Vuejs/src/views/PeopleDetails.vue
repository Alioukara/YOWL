...
<div class="detailsCard">
    ...
    <div class="detailsCard-grid">
        <img  class="detailsCard-image" :src="require(`@/assets/${$route.params.picturepeople}`)">
        <div class="detailsCard-header">
                  <h2>Specie:</h2>
                  <h3> {{ $route.params.specie }}</h3>
                  <h2>Gender:</h2>
                  <h3>{{ $route.params.gender }}</h3>
                  <h2>Hair color:</h2>
                  <h3>{{ $route.params.hair_color }}</h3>
                  <h2>Eye color:</h2>
                  <h3>{{ $route.params.eye_color }}</h3>
                  <h2>Birthyear:</h2>
                  <h3>{{ $route.params.birthyear }}</h3>
                  <h2>Height:</h2>
                  <h3>{{ $route.params.height }}</h3>
                  <h2>body mass:</h2>
                  <h3>{{ $route.params.body_mass }}</h3>
        </div>
      <div/>
      </div>
      <hr/>
     ...
     <hr>
      <div>
      <comments-card  v-for="(item, index) in listofcomments"
      :key="index"
      :comment="item"/>
     </div> 
...
export default {
  ...
  methods: {
    ...
    getAllcomments() {
       ...
       axios.get(process.env.VUE_APP_ENDPOINT +'/api/comments/search/people/'+ this.$route.params.id).then((response) => {
